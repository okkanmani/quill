...
def _generate_data_test_tier_passages(
    *,
    grade: int,
    tier: int,
    sitting_count: int,
    ...
    specs = _data_test_passage_specs(
        count=sitting_count,
        tier=tier,
    ...
    normalized = _normalize_data_draft(parsed, passage_specs=specs)
...
def _generate_adaptive_data_test_draft(
    *,
    grade: int,
    sitting_count: int,
    questions_per_passage: int,
    ...
    for tier in (1, 2, 3):
        tier_title, tier_passages = _generate_data_test_tier_passages(
            grade=grade,
            tier=tier,
            sitting_count=sitting_count,
            questions_per_passage=questions_per_passage,
    ...
    if not title:
        title = "Data Analysis Adaptive Test"
    return {"title": title, "passages": all_passages}


def _generate_fixed_data_test_draft(
    *,
    grade: int,
    sitting_count: int,
    questions_per_passage: int,
    ...
    title, passages = _generate_data_test_tier_passages(
        grade=grade,
        tier=2,
        sitting_count=sitting_count,
        questions_per_passage=questions_per_passage,
    ...
    *,
    ...
    if is_data:
        ...
        if not isinstance(per_passage, int) or per_passage < 1 or per_passage > 12:
            raise ValueError("questions_per_passage must be between 1 and 12.")
        if adaptive:
            return _generate_adaptive_data_test_draft(
                grade=grade,
                sitting_count=sitting_count,
                questions_per_passage=per_passage,
                custom_prompt=custom_prompt,
        ...
        return _generate_fixed_data_test_draft(
            grade=grade,
            sitting_count=sitting_count,
            questions_per_passage=per_passage,
            custom_prompt=custom_prompt,
