...
def _test_adaptive_from_sheet_data(data: dict) -> bool:
...
def _resolve_test(
    ...
    if data:
        ...
        adaptive = True if row_adaptive is None else bool(row_adaptive)
    meta: dict = {"is_test": flag}
    if flag:
        meta["test_sitting_count"] = sitting
        meta["test_adaptive"] = adaptive
    return meta
...
def save_focus_evaluation(result_id: int, student_name: str, data: dict) -> dict | None:
    ...
    try:
        ...
        from focus_reinforcement import should_trigger_reinforcement

        questions_by_area: dict[str, list[dict]] = {}
        for q in stored.get("questions") or []:
            ...
            if not area:
                continue
            questions_by_area.setdefault(area, []).append(q)
        ...
        for area, area_questions in questions_by_area.items():
            if should_trigger_reinforcement(area_questions):
