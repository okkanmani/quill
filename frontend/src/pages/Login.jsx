...
export default function Login() {
  ...
  const signupClosed = signupEnabled !== true;
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-semibold transition ${
              activeTab === TAB_SIGN_UP
                ? signupClosed
                  ? "bg-slate-100 text-slate-500 border-b-2 border-slate-300"
                  : "bg-indigo-50 text-indigo-900 border-b-2 border-indigo-600"
                : signupClosed
                  ? "text-slate-400 hover:bg-slate-50"
                  : "text-slate-600 hover:bg-slate-50"
            }`}
            aria-disabled={signupClosed}
          >
            Sign up
          </button>
        </div>
            ...
            <form onSubmit={handleSignup} className="flex flex-col gap-3">
              {signupClosed ? (
                <p className="text-slate-600 text-xs text-center leading-snug rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                  ...
                  Use the credentials you were given to sign in.
                </p>
              ) : (
                <p className="text-slate-600 text-xs text-center leading-snug">
              ...
              <button
                type="submit"
                disabled={busy || signupClosed}
                className="bg-slate-800 hover:bg-slate-900 text-white py-3 rounded-xl text-sm font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
