...
const LIBRARY_VARIANTS = {
  worksheets: {
    filter: (ws) => !ws.is_test,
    excludeRootModeKeys: ["tests"],
    pageTitle: "Worksheets",
    loadingLabel: "Loading worksheets…",
    emptyMessage: "No worksheets.",
    showThinkingQuest: true,
    showTopAddSection: true,
  ...
  tests: {
    ...
    rootModeKey: "tests",
    pageTitle: "Subject tests",
    loadingLabel: "Loading tests…",
    emptyMessage: "No subject tests.",
    showThinkingQuest: false,
    showTopAddSection: false,
    organizeLabel: "Organize unassigned tests into sections",
    itemSingular: "test",
    itemPlural: "tests",
    editPath: (ws) => `/admin/create/test?edit=${encodeURIComponent(ws.id)}`,
  },
};
...
export default function AdminWorksheetLibraryPage({
  variant = "worksheets",
  embedded = false,
}) {
  const config = LIBRARY_VARIANTS[variant] || LIBRARY_VARIANTS.worksheets;
  ...
  const visibleWorksheets = useMemo(
    () => worksheets.filter(config.filter),
    [worksheets, config],
  );

  const worksheetsNotInCollections = useMemo(
    () => unassignedWorksheets(visibleWorksheets),
    [visibleWorksheets],
  );

  const unassignedThinkingQuest = useMemo(
    () =>
      config.showThinkingQuest
        ? worksheetsNotInCollections.filter((ws) => ws.gifted_track)
        : [],
    [worksheetsNotInCollections, config.showThinkingQuest],
  );
  ...
  async function handleOrganizeUnassigned() {
    ...
    const organizePrompt =
      variant === "tests"
        ? `Create subject sub-sections under Tests from ${count} unassigned test(s)?\n\nYou can move tests afterward.`
    ...
    const ok = window.confirm(organizePrompt);
    if (!ok) return;
    setOrganizing(true);
    setError("");
    try {
  ...
  const itemWord = (count) => (count === 1 ? config.itemSingular : config.itemPlural);

  const libraryBody = (
      <div className={embedded ? undefined : "max-w-3xl"}>
        {!embedded ? (
          <h1 className={`${WS_PAGE_HEADING} mb-4`}>{config.pageTitle}</h1>
        ) : null}

        {!loading && !error && config.showTopAddSection ? (
          <div className="mb-6">
            <button
              type="button"
        ...
        {!loading && !error && visibleWorksheets.length === 0 && !worksheetSections.sections?.length && (
          <p className={WS_BODY}>{config.emptyMessage}</p>
        )}

        {!loading && !error && (visibleWorksheets.length > 0 || worksheetSections.sections?.length > 0) && (
          <>
            {anySelected ? (
              <div className="sticky top-0 z-30 -mx-1 mb-4 flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white/95 backdrop-blur-sm px-4 py-3 shadow-md">
                <span className={`${WS_CARD_TITLE} text-slate-800 tabular-nums`}>
                  {selectedCount > 0 && selectedSectionCount > 0
                    ? `${selectedCount} ${itemWord(selectedCount)}, ${selectedSectionCount} section${selectedSectionCount === 1 ? "" : "s"}`
                    : selectedCount > 0
                      ? `${selectedCount} ${itemWord(selectedCount)}`
                ...
                {selectedCount > 0 ? (
                  <>
                    <MoveActionButton
                      label={
                        moveSaving
                          ? `Moving selected ${config.itemPlural}…`
                          : `Move selected ${config.itemPlural}`
                      }
                      disabled={selectionBusy}
                    ...
                    <RecycleBinButton
                      label={
                        deleting
                          ? `Deleting selected ${config.itemPlural}…`
                          : `Delete selected ${config.itemPlural}`
                      }
                      disabled={selectionBusy}
            ...
            <AdminWorksheetCollectionTree
              ...
              rootModeKey={config.rootModeKey ?? null}
              excludeRootModeKeys={config.excludeRootModeKeys ?? null}
              defaultOpenDepth={variant === "tests" ? 0 : null}
              ...
              <div className="mt-2">
                <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                  ...
                  <OrganizeActionButton
                    label={
                      organizing
                        ? `Organizing ${config.itemPlural}…`
                        : config.organizeLabel
                    }
