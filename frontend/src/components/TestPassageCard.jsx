...
import { isTestQuestionComplete, TEST_TIERS } from "../testBuilderUtils";
...
export default function TestPassageCard({
  ...
  const passageComplete = Boolean(passage.title?.trim() && passage.body?.trim());
        ...
        <div className="p-4 border-t border-slate-100 space-y-4">
          ...
          <label className="block text-sm font-semibold text-slate-800">
            ...
            <textarea
              ...
              rows={8}
              placeholder="Paste or type the reading passage students will refer to."
              className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm resize-y min-h-[10rem] leading-relaxed"
            />
          </label>
          ...
          <div className="rounded-xl border border-indigo-100 bg-indigo-50/40 p-4 space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-slate-900">Questions for this passage</p>
                <p className="text-xs text-slate-600 mt-0.5">
                  {targetCount > 0
                    ? `Exactly ${targetCount} questions inherit this passage's tier (${tierLabel || "unset"}).`
                    : `All questions inherit this passage's tier (${tierLabel || "unset"}).`}
                </p>
              </div>
              {targetCount > 0 ? null : (
                <button
            ...
            {passageQuestions.length === 0 ? (
              <p className="text-sm text-slate-500 rounded-xl border border-dashed border-slate-200 px-4 py-6 text-center">
                {targetCount > 0
                  ? `This passage needs ${targetCount} questions — adjust questions per passage above if needed.`
